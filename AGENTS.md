...
- Run `uv run ruff check --fix .`, `uv run ruff format .`, `uv run pyrefly check`, `uv run mypy src/evm_rpc_picker`, `uv run pip-audit` and `uv run interrogate src/evm_rpc_picker` before finishing for linting, types, security and docstrings.
