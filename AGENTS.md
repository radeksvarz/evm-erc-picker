...
- Run `uv run ruff check --fix .` and `uv run ruff format .` before finishing for linting.
...
- **Checklist**: Always run `uv run ruff check --fix .`, `uv run ruff format .` and `uv run pytest` before committing.
